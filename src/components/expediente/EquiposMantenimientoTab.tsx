...
export function EquiposMantenimientoTab({ nave }: { nave: Nave }) {
  ...
  const { ordenesPorNave, sistemasPorNave, catalogoSistemaById } = useDataStore()
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {sistemas.map((s) => {
            const catalogo = catalogoSistemaById(s.sistemaId)
            return (
              <Card key={s.id}>
                <CardContent className="flex flex-col gap-2 p-4">
                  <div className="flex items-start justify-between gap-2">
                    <div className="text-sm font-semibold text-foreground">{catalogo?.nombre ?? s.sistemaId}</div>
                  ...
                  <div className="mt-1 flex flex-col gap-1 text-xs text-muted-foreground">
                    ...
                    <span>Frecuencia: {catalogo?.frecuencia ?? '—'}</span>
