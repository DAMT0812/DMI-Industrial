...
export const correctivosActivos = () => {
  const abiertas = ordenesTrabajo.filter((o) => o.estatus !== 'Validado' && o.estatus !== 'Cancelada')
  return {
...
export const capexProyectosMayores = () =>
  proyectosCapex.filter((p) => p.estatusComite === 'Aprobado por Dirección' || p.estatusComite === 'En Ejecución').length
...
export const slaPromedioResolucionHoras = () => {
  const cerradas = ordenesTrabajo.filter((o) => o.estatus === 'Validado' && o.fechaCierre)
