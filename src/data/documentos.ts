...
// El estatus del documento solo distingue vigente/vencido (el paso a "En Mora" es
// automático al cumplirse la fecha); qué tan cerca está de vencer lo calcula aparte
// alertas.ts sobre la misma fecha, no es parte del estatus del documento.
function estatusPorVencimiento(fechaVencimiento: string | null, moraSet?: Set<string>, naveId?: string): EstatusDocumental {
  if (moraSet && naveId && moraSet.has(naveId)) return 'En Mora'
  if (!fechaVencimiento) return 'Aprobado/Vigente'
  const dias = diasEntre(HOY, fechaVencimiento)
  if (dias < 0) return 'En Mora'
...
export const documentos: DocumentoPermiso[] = naves.flatMap((nave, idx) => {
  ...
  docs.push({
    ...
    estatusJuridico: 'Aprobado/Vigente',
